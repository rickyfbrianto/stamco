...
function AddToBasketProduct({ product, disabled }: AddToBasketProps) {
    ...
    const handleAddToCart = async (product: Product) => {
        if (userId) {
            setLoading(true)
            updateIncQtyCart(product, userId, watchQty)
                .then(() => {
                    setLoading(false)
    ...
    return (
        <Controller name="quantity" control={form.control} render={({ field: { value, onChange } }) => (
            <div className="gap-2 mt-2 w-full bg-white fixed flex flex-col justify-center items-center bottom-0 left-0 p-2 border-t-[2px]
